...
class TestRoles(SPTestCase):
    ...
    def test4_add_role_assignment(self):
        ...
        result = self.client.web.add_role_assignment(target_user, self.__class__.target_object).execute_query()
        self.assertIsNotNone(result.resource_path)

    def test5_remove_role_assignment(self):
        target_user = self.client.web.current_user
        result = self.client.web.remove_role_assignment(target_user, self.__class__.target_object).execute_query()
        self.assertIsNotNone(result.resource_path)
